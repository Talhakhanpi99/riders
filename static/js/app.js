...
async function toggleOfflineListening() {
  ...
  try {
    ...
    document.querySelector("#offlineListenButton").textContent = offlineListening ? "Background assistant enabled" : "Enable Background Assistant";
    responseText.textContent = payload.message || (offlineListening ? "Background listening started." : "Background listening stopped.");
    setState(offlineListening ? "Background listening" : "Ready", offlineListening);
  } catch (error) {
